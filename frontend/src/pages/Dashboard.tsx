...
export default function Dashboard() {
  ...
  const steps = useMemo(() => demo?.scenarios.flatMap((s) => s.steps.map((st) => ({ ...st, scenario: s.id }))) ?? [], [demo]);
    ...
    <div>
      <div className="topbar">
        <div>
          ...
          <p className="subtitle">AI proposes. MoneyGuard evaluates. Policy authorizes. Simulator executes in test mode.</p>
        </div>
        <button className="btn" disabled={busy} onClick={runDemo}>
          {busy ? "Running live demo…" : "Run live demo"}
        </button>
      </div>
      <div className="banner">This prototype uses synthetic/test-mode financial data and does not move real money.</div>
      ...
      <div className="kpis">
        ...
        <div className="card kpi">
          ...
          <div className="value">{metrics?.transactions ?? 0}</div>
        </div>
            ...
            {demo.scenarios.map((s) => (
              <div key={s.id}>
                ...
                <div className="pipeline">
                  {s.steps.map((st, idx) => {
                    const globalIndex = steps.findIndex((x) => x === st || (x.title === st.title && x.scenario === s.id && x.decision === st.decision));
                    const shown = visible > idx || visible >= s.steps.length;
                    const shownSafe = steps.slice(0, visible).some((x) => x.title === st.title && x.scenario === s.id);
                    if (!shownSafe && !shown) return null;
                    return (
                      <div className="stage" key={`${s.id}-${idx}`}>
                        <h3>{st.title}</h3>
                        <div className="mono">{st.agent} · conf {(st.confidence * 100).toFixed(0)}%</div>
                        <div>{st.decision}</div>
      ...
      <div className="card">
        ...
        <table>
          ...
          <tbody>
            ...
            {incidents.length === 0 && (
              <tr>
                <td colSpan={7} className="subtitle">No incidents yet. Run the live demo to generate synthetic leakage and recovery.</td>
              </tr>
            )}
          </tbody>
        </table>
...
function Chart({ title, data, color }: { title: string; data: { t: string; v: number }[]; color: string }) {
  return (
    <div className="card">
